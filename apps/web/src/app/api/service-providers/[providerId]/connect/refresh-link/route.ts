...
/**
 * BFF Stripe Connect link-refresh proxy — regenerates an Account Link whose
 * predecessor expired (they live only minutes).
 *
 * `POST /api/service-providers/{providerId}/connect/refresh-link` →
 * `POST /service-providers/{id}/connect/refresh-link`. Authenticated server-side by the
