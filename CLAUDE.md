...
- **Typage JSONB free-form (`Record<string, never>`) — partiellement résolu (3.12a-back)** : un `@ApiProperty` de map JSONB **sans** `additionalProperties` génère `Record<string, never>` (inexploitable) côté `schema.d.ts`. **Corrigé à la source** pour `GET /service-providers/{id}/service-requests` (`additionalProperties: { type: 'string' }` → `serviceCategoryNameTranslations`/`serviceItemNameTranslations` typés `Record<string, string>`). **Reste à ré-annoter** (toujours `Record<string, never>` dans `schema.d.ts`) : `AdminVerificationQueueItemDto.serviceCategoryNameTranslations`, `authorityFullNameTranslations`, et les autres maps des endpoints antérieurs — la correction ne vaut que pour le nouvel endpoint pour l'instant.
