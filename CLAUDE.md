...
**3.12b-PR2-étape1** (front) : **composant `ConfirmDialog` réutilisable** (`apps/web/src/components/ui/confirm-dialog.tsx`, premier composant du dossier partagé `components/ui/` créé à cette occasion). Socle de confirmation bâti sur l'élément **`<dialog>` natif** (`showModal()`/`close()` pilotés par `useRef`+`useEffect` synchronisés sur `isOpen`) — focus trap, inertie du fond et Échap portés par la plateforme, non réimplémentés. **Props** : `isOpen`, `onClose`, `title`, `children` (corps injecté), `confirmLabel`, `onConfirm: () => Promise<void>`. **Orchestration** : le dialogue gère lui-même l'état `pending` (boutons verrouillés → anti double-clic ; Échap/overlay neutralisés pendant l'action) ; `onConfirm` résout → fermeture, rejette → reste ouvert et affiche `error.message` **verbatim**. **Agnostique du métier (décision verrouillée)** : aucun code HTTP, aucun mapping — le mapping FR des erreurs vit dans le *consommateur* (préparé pour l'i18n future). Deux consommateurs à venir (modale Accept, AlertDialog Refuser). Pièges gérés : guard `dialog.open` avant chaque appel impératif ; `preventDefault` sur `onCancel` pour ne pas désync le DOM de `isOpen` ; **`m-auto`** pour restaurer le centrage UA cassé par le reset `margin:0` de Tailwind v4 Preflight (à réappliquer sur tout futur `<dialog>` natif). Harnais de test jetable `app/_dev/confirm-dialog` (**dette : à purger en fin de PR 2**). Validé : revue + test navigateur (succès→ferme, échec→message persistant, Annuler ferme, mode sombre). **Étapes suivantes PR 2** : `AcceptRequestAction` (modale riche + mapping 409/502/422), `DeclineRequestAction`, `JobPipelineAction`, câblage dans les cartes.
**3.12b-PR2-étape2** (front) : **composant `AcceptRequestAction`** (`apps/web/src/app/dashboard/_actions/accept-request-action.tsx`) — premier consommateur de `ConfirmDialog`, bouton « Accepter » d'une demande OPEN ciblée ouvrant une modale riche (titre, client, adresse, montant total estimé formaté `fr-CA`). **Effet financier réel** assumé : l'accept capture un dépôt Stripe côté backend. `onConfirm` appelle le BFF `POST /api/service-requests/{id}/accept`, puis **succès → `router.refresh()`** (le Server Component re-fetch, la demande migre OPEN→ASSIGNED). **Mapping erreurs par statut HTTP SEUL** (décision verrouillée, corps jamais parsé) : table FR figée en vouvoiement (409 disponibilité/paiement, 502 prélèvement échoué, 422 montant manquant, 404 inaccessible, fallback réseau/5xx) affichée verbatim par `ConfirmDialog`. **Taux de dépôt (20 %) JAMAIS affiché** (décision Option 2) : seul le montant total est surfacé, la phrase annonce « un dépôt » sans chiffre ni pourcentage. **Garde `estimatedAmount == null` en défense en profondeur** : (1) UX via nouvelle prop `confirmDisabled` sur `ConfirmDialog` (**strictement rétro-compatible**, défaut `false` → comportement mergé inchangé, non-régression vérifiée sur l'ancien harnais), bouton confirm désactivé + avertissement ambre ; (2) correctness, `onConfirm` refuse localement (throw 422) → jamais d'accept voué au 422. Validé : revue code (mapping byte-exact) + test navigateur (5 messages verbatim, cas null bouton grisé, succès ferme). Harnais jetable `app/_dev/accept-action` (**dette : à purger en fin de PR 2, avec celui de confirm-dialog**). **Étapes suivantes PR 2** : `DeclineRequestAction` (AlertDialog léger), `JobPipelineAction` (Démarrer/Compléter clic direct), câblage dans `PendingRequestCard`/`JobCard`.
**3.12b-PR2-étape3** (front) : **composant `DeclineRequestAction`** (`apps/web/src/app/dashboard/_actions/decline-request-action.tsx`) — deuxième consommateur de `ConfirmDialog`, bouton « Refuser » (style outline zinc discret, secondaire vs l'accent de l'Accept) d'une demande OPEN ciblée. **Financièrement INERTE** : le refus fait passer la demande OPEN→CANCELLED côté backend, zéro Stripe → **confirmation LÉGÈRE** (une phrase « Êtes-vous sûr de vouloir refuser cette demande ? Cette action est définitive. »), pas de modale riche (friction proportionnelle au risque). **Refus SANS motif** (décision produit) : `POST /api/service-requests/{id}/decline` **sans corps** (le champ `reason` optionnel de l'API délibérément inutilisé ; réserve pour le jour où une feature consomme le motif). Succès → `router.refresh()`. **Mapping erreurs par statut HTTP seul** (verrouillé) : table FR figée vouvoiement, 3 cas seulement — 409 (« plus disponible et ne peut être refusée »), 404 (« plus accessible », identique à l'Accept — cohérence de voix), fallback réseau/autre. **Pas de 502/422 mappés** (un refus ne touche pas Stripe, n'exige aucun montant ; ces codes ne peuvent survenir → un 502 éventuel retombe sur le `default`). **`ConfirmDialog` NON modifié** (consommé tel quel, `confirmDisabled` non utilisé). Validé : revue code (mapping byte-exact, fetch sans corps prouvé) + test navigateur (3 messages verbatim, succès ferme). Harnais jetable `app/_dev/decline-action` (**dette : à purger en fin de PR 2, avec les 2 autres harnais**). **Étapes suivantes PR 2** : `JobPipelineAction` (Démarrer/Compléter clic direct), câblage dans `PendingRequestCard`/`JobCard`, puis purge des 3 harnais `_dev`.
