...
- **Découverte géo (3.14a) — dettes de suivi** : (1) **pagination différée** sur `/recherche` (`page=1&limit=50`, **un seul appel**, aucune UI de pagination — même dette que le dashboard prestataire et `/requests`) ; (2) **3.14b** — **géocodage d'adresse** (champ adresse → coords) : `/recherche` ne prend aujourd'hui que `navigator.geolocation` ; **et threading des vraies coords dans la création de demande** (§3.13 `create-request-form.tsx` pose encore le placeholder Québec fixe `serviceLocation = [-71.21, 46.81]` — à **tuer** en 3.14b) ; (3) **découverte publique/anonyme** = chantier futur distinct (rendre `discover` `@Public()` + rate-limit ; aujourd'hui **JWT requis**, `/recherche` est **privée** sous `(app)`).
- **Géocodage (3.14b-back) — dettes de suivi** : la couture `GET /geocode` (proxy Nominatim, port/adapter `common/geocoding/`) est **livrée** ; restent différés — (1) **cache Redis** du géocodage (recommandé par le ToS Nominatim ; différé car volume MVP faible, le `User-Agent` correct suffit à ce volume) ; (2) **rate-limiter** sur `/geocode` (Nominatim ≈ 1 req/s) — différé ; (3) **2ᵉ adapter** (Mapbox/Google) prêt à brancher derrière la couture via `GEOCODING_PROVIDER` si volume/autocomplete l'exigent (jamais un rewrite) ; (4) **3.14b-front** — comment le navigateur atteint `/geocode` alors que le token est **httpOnly server-side** (même classe de problème que la voie Ⓒ de 3.14a : URL + Server Component, ou route handler BFF ?), **et** le threading des vraies coords dans la création de demande (`create-request-form.tsx` pose encore `serviceLocation = [-71.21, 46.81]` fixe → à **tuer** en 3.14b-front, cf. bullet 3.14a item (2) ci-dessus).
