...
**Dernière session :** 2026-08-19 (session 7) · `main` @ `752b340` · migration `1780510000000` (inchangée)
...
Prochain chantier de la file : **G-2**, puis **D — avis et évaluations**.
...
1. **⚠️ LA MENACE DE VIE PRIVÉE DE `demand_signals` A CHANGÉ DE CAMP — elle vit désormais dans la LECTURE.**
   ...
   **aucune méthode ne rend une ligne individuelle**. Ajouter un `findAll()` « pour déboguer » annulerait
   la session 6 **sans un seul symptôme**.
...
3. **Le contrat propre en amont paie immédiatement.** `@ApiProperty` à **type concret** sur chaque
   ...
   fatalité.
4. **⚠️ NOUVEAU PIÈGE `pkill`/`grep`, variante de la note session 6.** Un `for p in $(ps … | grep "[p]roxy" …)`
...
**② Conservation d'une recherche restée vide — CLOSE depuis la session 6, et pas par un chiffre.**
...
agrégée et tronquée au jour, elle **n'introduit aucun nouveau champ conservé**. *Aucune action requise.*

**③ Faut-il inscrire des invariants dans `CLAUDE.md` §13.1 ? — élargie une troisième fois.**
Modifier §13.1 est sur la liste d'arrêt (mandat §3.9), donc je ne l'ai pas fait. **Trois** candidats, tous
du profil « le code ressemble à un oubli, le corriger casse tout sans symptôme » :
- **(a)** « offert ≠ résolvable » → `lib/catalog/launch-scope.ts` + commentaire de `recherche/page.tsx`.
- **(b)** « `demand_signals` ne porte aucun identifiant, et l'arrondi `numeric(_,2)` est porteur »
  → migration `1780510000000` + entité.
- **(c) NOUVEAU** « la lecture de `demand_signals` est agrégée en SQL et tronquée au jour — aucune méthode
  ne rend une ligne » → `demand-signals.repository.ts` + `DemandSignalSummaryItemDto`. **(b) protège le
  schéma, (c) protège la lecture ; (b) seul ne suffit pas**, c'est la démonstration de la session 7.
- **Option recommandée : oui, ajouter les trois**, en entrées courtes renvoyant au code. En attendant, la
  protection ne vit que dans les commentaires et les entrées §6/§11.

**④ NOUVEAU — la limitation de débit sur `POST /demand-signals` monte en priorité.**
Elle était une nuisance théorique tant que personne ne lisait la table. Depuis #77, **une décision de
recrutement s'appuiera sur ces chiffres** : la vérification serveur empêche d'inventer un métier ou un
secteur **non vides**, elle n'empêche pas un compte authentifié de **gonfler** un secteur réellement vide.
- **Option recommandée : la traiter avec le chantier « rendre `discover` public + limitation de débit »**,
  qui posera le même besoin au même endroit — et **pas** en la bricolant seule sur une route.
...
**Relevées, inchangées** : limitation de débit sur l'écriture (voir ⛔④) · `locality` au géocodage —
**la vraie réponse au « où » lisible, deuxième fois qu'elle est le bon correctif** · le foyer *feature
toggling* · les 5 occurrences restantes de « tableau de bord » · `body { font-family: Arial… }` dans
`globals.css` écrase les polices Geist (PR dédiée) · aucune vérification en **mode sombre** nulle part ·
les deux liens en ligne à 16 px de #73 · cibles tactiles **dans les pages** non auditées (G-2) ·
notifications prestataire ORGANIZATION illisibles · `docker/data/` tracké par git.
